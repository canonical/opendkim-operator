...
- Built snap artifact: `opendkim-snap/opendkim_2.11.0+snap2_amd64.snap`
...
```text
opendkim_2.11.0+snap2_amd64.snap
```
...
sudo snap install --dangerous --devmode ./opendkim_2.11.0+snap2_amd64.snap
```
...
Run snap-native tests with `snapcraft test`:
...
This runs the Spread task under `tests/spread/smoke` and validates:

- snap build (`snapcraft pack`)
- local install of the produced snap
- `opendkim.genkey` and `opendkim.testkey` smoke checks
- generated `default.private` and `default.txt`

To run only the smoke suite:

```bash
cd opendkim-snap
snapcraft test tests/spread/smoke
```
