...
# snapcraft init cannot run in a directory that already has a spread.yaml,
# so we use a temporary directory and copy only the .extension file.
tmpdir=$(mktemp -d)
...
The `spread/.extension` helper script is not checked into the repository — it is
generated by `snapcraft init --profile test` so that we always use the upstream version.
