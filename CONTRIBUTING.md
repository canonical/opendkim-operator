...
- Generally, before developing enhancements to this charm, you should consider [opening an issue
  ](https://github.com/canonical/opendkim-operator/issues) explaining your use case.
...
- [Fork](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/working-with-forks/about-forks)
  our [GitHub repository](https://github.com/canonical/opendkim-operator)
...
# Generate the spread/.extension helper (first time only).
# snapcraft test requires this file on disk — it hardcodes references to
# ./spread/.extension in the backend config it passes to spread, but does
# not generate it automatically. The only way to obtain the file is via
# snapcraft init --profile test, which refuses to run in a directory that
# already contains a spread.yaml, so we use a temporary directory and copy
# only the .extension file.
tmpdir=$(mktemp -d)
mkdir -p "$tmpdir/snap" && cp snap/snapcraft.yaml "$tmpdir/snap/"
snapcraft init --profile test "$tmpdir"
cp "$tmpdir/spread/.extension" spread/.extension
rm -rf "$tmpdir"

snapcraft test
```

This builds the snap and then runs all spread tests inside an LXD VM.
The `spread/.extension` helper script is not checked into the repository. `snapcraft
test` expects this file to exist but does not generate it — it hardcodes references
to `./spread/.extension` in the spread backend configuration. We generate it from the
upstream template using `snapcraft init --profile test` so it stays in sync with the
installed snapcraft version.
